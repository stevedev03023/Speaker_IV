...
def embed_speaker(wavs, using_partials=True, **kwargs):
    # Process the entire utterance if not using partials
    frames_batch = []
    for wav in wavs:
        if not using_partials:
            frames = audio.wav_to_mel_spectrogram(wav)
            frames_batch.append(np.array([frames]))
        else:
            # Compute where to split the utterance into partials and pad if necessary
            wave_slices, mel_slices = compute_partial_slices(len(wav), **kwargs)
            max_wave_length = wave_slices[-1].stop
            ...
            # Split the utterance into partials
            frames = audio.wav_to_mel_spectrogram(wav)
            frames_batch.append(np.array([frames[s] for s in mel_slices]))

    frames_batch = np.concatenate(frames_batch)
    embeds = embed_frames_batch(frames_batch)
