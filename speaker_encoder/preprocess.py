...
def preprocess_speaker(sample):
    speaker_dir, (datasets_root, out_dir, extension, skip_existing, logger) = sample
...
def _preprocess_speaker_dirs(speaker_dirs, dataset_name, datasets_root, out_dir, extension,
    ...
    cnt = len(speaker_dirs)
    # Process the utterances for each speaker
    with Pool(8) as pool:
        list(tqdm(pool.map(preprocess_speaker, zip(speaker_dirs, [(datasets_root, out_dir, extension, skip_existing, logger)]*cnt)), dataset_name, cnt,
                  unit="speakers"))
    logger.finalize()
